...
/* rivo does not author edits. Each box composes an instruction and the range it
...
async function showPicker(session) {
  ...
  if (!ready.length) {
    ...
    session.pickEl.innerHTML = '<div class="hint" style="line-height: 1.5; padding: 4px 2px;">' +
      ...
      'Please install a coding harness, make sure it is on your <code>PATH</code>, and restart rivo after that.</div>';
    return;
  }
