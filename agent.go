...
type agentJob struct {
	...
	done   chan struct{} // closed after the harness and post-run bookkeeping stop
	out    *tailBuffer
	stderr *tailBuffer
	start  time.Time
}
...
func (m *agentManager) Close() {
	...
	m.mu.Unlock()
	// Wait for every harness and its post-run bookkeeping before exiting.
...
// rivo dispatched the harness, so it knows when the work ended without
