...
This document explains the technical implementation of rivo's in-memory indexer ([`index.go`](../../index.go)) and custom `.gitignore` evaluation engine ([`ignore.go`](../../ignore.go)).
...
Unbounded goroutine creation during directory traversal can cause thread contention, high memory churn, and "too many open files" errors. rivo controls traversal concurrency using a buffered channel semaphore:
...
Evaluating regular expressions for hundreds of `.gitignore` patterns against tens of thousands of paths can cripple indexing performance. In [`ignore.go`](../../ignore.go), rivo classifies patterns into specialized non-regex fast paths and adds heuristic pre-filters to regexes.
...
rivo implements a clear distinction between ignored entries and version control internals:
