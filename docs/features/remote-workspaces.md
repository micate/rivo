...
rivo is architected from the ground up as a remote-first code reading environment. It allows you to run a single binary on any remote server, cloud instance, Docker container, or CI runner and browse the codebase directly in your local desktop browser without SSH keys, port forwarding setups, or heavy remote extension daemons.
...
- **Single Port Operation**: rivo serves all assets, JSON APIs, and search queries over a single HTTP port (default `7777`).
...
- **Direct Terminal Ergonomics**: Launch rivo targeting specific files or line numbers directly from the command line:
  - `rivo` (opens current directory)
  - `rivo ~/projects/kernel` (opens specified repository)
  - `rivo main.go:42` (opens directly to line 42)
- **Built-in Self-Updater (`rivo --update`)**: Checks GitHub releases and seamlessly upgrades the single binary in place.
...
Run rivo on your cloud instance bound to all interfaces:
```bash
...
```bash
docker run -p 7777:7777 -v $(pwd):/workspace rivo:latest -host 0.0.0.0 /workspace
```

### 3. CI/CD Runner Debugging
When a build or test suite fails on a remote CI runner, download rivo, run it in the background, and inspect generated artifacts, failure logs, and git status directly in your browser.
