...
rivo is an ultra-fast, zero-config code reader and navigator specifically optimized for fast reads, remote inspection, and AI-assisted workflows. It provides instant access to codebases of any size with minimal resource consumption (~20 MB RAM, < 1 ms startup).

This directory provides comprehensive documentation for all rivo user-facing features, detailing how each capability works, its role in developer and AI agent pairing workflows, interactive controls, and configuration options.
...
Fast code discovery is essential when reviewing large codebases or inspecting agent changes. rivo provides three complementary navigation layers:
...
Understanding complex software systems requires more than raw text search. rivo delivers structural and semantic code intelligence while maintaining an ultra-lightweight footprint:
...
rivo intentionally omits a heavyweight text editor in favor of direct collaboration with the user's preferred coding agents:

- **[Editing with Coding Agents](agent-editing.md)**: Select any code block in a source file or git diff, press `Alt+E` (or right-click), describe the required change, and rivo delegates the task directly to your chosen agent harness (Claude Code, Gemini CLI, Cursor Agent, Antigravity, OpenCode, Codex, Aider, or Goose). Execution progress streams to the launch terminal, while rivo automatically detects file changes and reloads tabs in place upon completion.
...
rivo adapts to developer habits and viewing environments without requiring manual config file tinkering:

- **[Settings & Preferences System](settings-and-configuration.md)**: Comprehensive settings manager accessible via `Cmd/Ctrl+,`. Offers a VS Code-style graphical UI with interactive attribute pills alongside a raw JSON editor (`~/.rivo/settings.json`) that updates themes, typography, diff layouts, and search behavior in real time without refreshing the browser.
...
- **[Remote Workspaces & Cloud Inspection](remote-workspaces.md)**: Run rivo on remote servers, cloud VMs, Docker containers, or CI runners and view code in your local browser over Tailscale or private networks without SSH keys, port forwarding setups, or remote desktop daemons.
