...
// Closing a desktop workspace must stop every concurrent harness before return.
func TestAgentCloseWaitsForAllJobs(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "file.txt"), []byte("one\ntwo\n"), 0600); err != nil {
		t.Fatal(err)
	}
	s := agentServer(t, root, writeHarness(t, "exec sleep 30\n"))
	var ids []int64
	for line := 1; line <= 2; line++ {
		job, err := s.agent.Start(filepath.Join(root, "file.txt"), "file.txt", line, line, "edit", false)
		if err != nil {
			t.Fatal(err)
		}
	...
	go func() { s.agent.Close(); close(closed) }()
	select {
	case <-closed:
	case <-time.After(5 * time.Second):
		t.Fatal("Close did not stop all harnesses")
	}
	for _, id := range ids {
		if s.agent.Job(id).Running {
			t.Fatalf("job %d still running after Close", id)
		}
		select {
		case <-s.agent.jobs[id].done:
		default:
			t.Fatalf("job %d bookkeeping still running after Close", id)
		}
	}
}
